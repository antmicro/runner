    ...
    public sealed class RunServer : RunnerService, IRunServer
    {
        ...
        public Task<AgentJobRequestMessage> GetJobMessageAsync(string id, string billingOwnerId, CancellationToken cancellationToken)
        {
            ...
            return RetryRequest<AgentJobRequestMessage>(
                async () => await _runServiceHttpClient.GetJobMessageAsync(requestUri, id, billingOwnerId, cancellationToken), cancellationToken,
                shouldRetry: ex =>
            ...
            CancellationToken cancellationToken)
        {
            CheckConnection();
            return RetryRequest(
                async () => await _runServiceHttpClient.CompleteJobAsync(requestUri, planId, jobId, result, outputs, stepResults, billingOwnerId, cancellationToken), cancellationToken);
        }
